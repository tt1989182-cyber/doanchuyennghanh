...
@endsection
@section('title','AURA || CHI TIẾT SẢN PHẨM')
...
@endphp
<p class="price">
    <span class="discount">{{ number_format($after_discount, 0, ',', '.') }} ₫</span>
    <s>{{ number_format($product_detail->price, 0, ',', '.') }} ₫</s>
</p>
											</div>
											<!--/ End Description -->
											...
											<div class="product-buy">
												...
												@endif
												<p class="availability">Còn hàng : @if($product_detail->stock>0)<span class="badge badge-success">{{$product_detail->stock}}</span>@else <span class="badge badge-danger">{{$product_detail->stock}}</span>  @endif</p>
											</div>
											<!--/ End Product Buy -->
										</div>
									</div>
														...
														<div class="row">
															<div class="col-12">
																...
																<p class="text-center p-5">
																	Bạn cần <a href="{{route('login.form')}}" style="color:rgb(54, 54, 204)">Đăng nhập</a> HAY <a style="color:blue" href="{{route('register.form')}}">Đăng kí</a>
                                    ...
                                    <div class="product-img">
										<a href="{{route('product-detail',$data->slug)}}">
                                            ...
                                            <span class="price-dec">{{$data->discount}} % Off</span>
                                                                    {{-- <span class="out-of-stock">Nổi bật</span> --}}
                                        </a>
...
@endpush
@push('scripts')
 <!-- model-viewer (thư viện chính) -->
<script type="module" src="https://unpkg.com/@google/model-viewer/dist/model-viewer.min.js"></script>

<script>
/**
 * Khi modal đóng -> dừng bất kỳ session AR/animation đang chạy.
 * model-viewer có method .stop() không chuẩn, nhưng ta có thể
 * gọi leave AR bằng attribute .cancel or set camera? Ở đây ta dừng autoplay bằng reload source khi đóng.
 */
function stopAR(){
    const mv = document.getElementById('modelViewer');
    if(!mv) return;
    try{
        // reset src để dừng bất kỳ session nào (an toàn)
        const cur = mv.getAttribute('src');
        mv.removeAttribute('src');
        // small delay rồi restore src (để lần mở sau vẫn load)
        setTimeout(()=>{ mv.setAttribute('src', cur); }, 100);
    }catch(e){
        console.log('stopAR error', e);
    }
}

// Khi modal đóng bằng nút hoặc backdrop
$('#arViewerModal').on('hidden.bs.modal', function () {
    stopAR();
});
</script>
