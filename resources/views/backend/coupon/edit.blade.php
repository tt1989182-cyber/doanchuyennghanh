...
<div class="card">
    <h5 class="card-header">Chỉnh sửa mã giảm</h5>
    <div class="card-body">
      <form method="post" action="{{route('coupon.update',$coupon->id)}}">
        @csrf 
        @method('PATCH')
        <div class="form-group">
          <label for="inputTitle" class="col-form-label">Mã giảm <span class="text-danger">*</span></label>
          ...
          <div class="form-group">
              ...
              <select name="type" class="form-control">
                  <option value="fixed" {{(($coupon->type=='fixed') ? 'selected' : '')}}>Chỉnh sửa</option>
                  <option value="percent" {{(($coupon->type=='Phần trăm') ? 'selected' : '')}}>Phần trăm</option>
              </select>
              @error('type')
              <span class="text-danger">{{$message}}</span>
              @enderror
