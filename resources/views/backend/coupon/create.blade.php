...
<div class="card">
    <h5 class="card-header">Thêm mã giảm</h5>
    <div class="card-body">
      <form method="post" action="{{route('coupon.store')}}">
        ...
        <div class="form-group">
            ...
            <select name="type" class="form-control">
                <option value="fixed">Chỉnh sửa</option>
                <option value="percent">Phẩn trăm</option>
            </select>
            @error('type')
            <span class="text-danger">{{$message}}</span>
            @enderror
        ...
        <div class="form-group mb-3">
          <button type="reset" class="btn btn-warning">Đặt lại</button>
           <button class="btn btn-success" type="submit">Tóm tắt</button>
        </div>
      </form>
    </div>
</div>
