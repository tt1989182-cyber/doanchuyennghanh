...
{
   public function arView($slug)
{
    $product_detail = Product::where('slug', $slug)->firstOrFail();
    return view('frontend.pages.ar-view', compact('product_detail'));
}
    ...
    $category_lists = Category::where('status','active')->where('is_parent',1)
                        ->orderBy('title','ASC')->get();

    return view('frontend.index', compact('featured','banners','product_lists','category_lists'));
}
    ...
    public function productGrids(){
       // $products=Product::query();
         $products = Product::where('status','active'); // luôn lọc active từ đầu
    ...
    public function loginSubmit(Request $request){
        ...
        if(Auth::attempt(['email' => $data['email'], 'password' => $data['password'],'status'=>'active'])){
            ...
            request()->session()->flash('error','Sai email hoặc mật khẩu, vui lòng thử lại!');
